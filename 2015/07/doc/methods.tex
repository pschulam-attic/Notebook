...
%We note that more difficult challenge is specifying the joint distribution over the auxiliary process latent variables $p(\bm{z}_{1:M})$ as it's unclear how it should be factorized and simplified. On the other hand, the conditional distribution $p(\bm{z} \given \bm{z}_{1:M})$ is slightly less challenging because we can simply assume that $\bm{z}$ depends on all auxiliary latent variables. The issue here becomes choosing a parameterization of the conditional distribution that is sufficiently flexible to roughly capture the functional form of the dependencies.
